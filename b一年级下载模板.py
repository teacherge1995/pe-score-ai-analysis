...
def generate_1_grade_template(school_name, header_name, selected_items, boy_count, girl_count, save_path,file_name):
    """
    生成一年级体育成绩模板
    ...
    for i in range(int(boy_count)):
        ...
        if '跳绳' in selected_items:
            ws.cell(row=row, column=5, value=160)
        if '坐位体前屈' in selected_items:
            ws.cell(row=row, column=6, value=20)
        if '肺活量' in selected_items:
            ws.cell(row=row, column=7, value=1800)
        if '身高' in selected_items:
            ws.cell(row=row, column=8, value=150)
        if '体重' in selected_items:
            ws.cell(row=row, column=9, value=34)
    ...
    for col in range(1, 10):  # A-I列
        ...
        cell.border = Border(
            ...
            right=double_side if col == 9 else thin_side  # 最后一列右边框为双线
        )
    ...
    for row in range(5, 33):  # 5-32行
        for col in range(1, 10):  # A-I列
            ...
            right = double_side if col == 9 else thin_side
    ...
    # 文件名是字符串（如"一年级1班"），无需转float，且仅操作合并区域左上角
    ws2['G2'] = file_name  # 去掉float转换，直接赋值字符串
    ws2['G2'].font = Font(size=12, bold=True)
    ws2['G2'].alignment = Alignment(horizontal='center', vertical='center')
    # 文件名是字符串（如"一年级1班"），无需转float，且仅操作合并区域左上角
    ws2['G34'] = file_name  # 去掉float转换，直接赋值字符串
    ws2['G34'].font = Font(size=12, bold=True)
    ws2['G34'].alignment = Alignment(horizontal='center', vertical='center')
