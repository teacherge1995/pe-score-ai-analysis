from zgFeiHuoLiang_BiaoZhun import yinianji_nan_feihuoliang_jisuan_fenshu, yinianji_nv_feihuoliang_jisuan_fenshu
from zbTiaoSheng_BiaoZhun import yinianji_nan_tiaosheng_jisuan_fenshu, yinianji_nv_tiaosheng_jisuan_fenshu
from zcZuoWeiTiQianQu_BiaoZhun import yinianji_nan_zuoweitiqianqu_jisuan_fenshu, yinianji_nv_zuoweitiqianqu_jisuan_fenshu
from zaWuShiMi_BiaoZhun import yinianji_nan_wushimi_jisuan_fenshu, yinianji_nv_wushimi_jisuan_fenshu
from zhBMI_BiaoZhun import 一年级男生体重指数计算分数,一年级女生体重指数计算分数
...
def JiSuan_nan_list(data_list, wenjianming):
    ...
    for row in data_list:
        try:
            ...
            # ====================== 原有男生判断逻辑 ======================
            if gender in ['男', '男生']:
                Bmi = round((float(row[8])) / (((float(row[7])) / 100) * ((float(row[7])) / 100)), 1)

                wushimi_func = yinianji_nan_wushimi_jisuan_fenshu
                tiaosheng_func = yinianji_nan_tiaosheng_jisuan_fenshu
                zuoweitiqianqu_func = yinianji_nan_zuoweitiqianqu_jisuan_fenshu
                feihuoliang_func = yinianji_nan_feihuoliang_jisuan_fenshu
                Bmi_func = 一年级男生体重指数计算分数(Bmi)
                xuhao = row[0]
                xingming = row[1]
                xingbie = row[2]
                banji = wenjianming
                ...
                try:
                    ...
                    sanxiangpingjunfen = round((wushimi_defen * 0.2 +
                                                ...
                                                zuoweitiqianqu_defen * 0.2) +
                                               tiaosheng_jiafen, 2)
                    sanxiangdengji = zuihoudengji70_panduan(sanxiangpingjunfen)
                except Exception as e:
                    raise Exception(f"【三项总分计算错误】男生姓名：{xingming}，序号：{xuhao}，<br>错误信息：{str(e)}")

                # ====================== 肺活量 ======================
                try:
                    feihuoliangchengji = float(row[6])
                    feihuoliang_defen = feihuoliang_func(float(row[6]))[0]
                    feihuoliang_dengji = feihuoliang_func(float(row[6]))[1]
                except Exception as e:
                ...
                # 加上肺活量 bmi 5项
                try:
                    wuxiangzongfen = wushimi_defen + tiaosheng_defen + zuoweitiqianqu_defen + feihuoliang_defen + Bmi_defen
                    wuxiangpingjunfen = round((Bmi_defen * 0.15 +
                                               feihuoliang_defen * 0.15 +
                                               wushimi_defen * 0.2 +
                                               (tiaosheng_defen - tiaosheng_jiafen) * 0.3 +
                                               zuoweitiqianqu_defen * 0.2) +
                                              tiaosheng_jiafen, 2)
                    wuxiangdengji = zuihoudengji_panduan(wuxiangpingjunfen)
                except Exception as e:
    ...
    for row in data_list:
        try:
            ...
            if gender in ['女', '女生']:
                ...
                wushimi_func = yinianji_nv_wushimi_jisuan_fenshu
                tiaosheng_func = yinianji_nv_tiaosheng_jisuan_fenshu
                zuoweitiqianqu_func = yinianji_nv_zuoweitiqianqu_jisuan_fenshu
                feihuoliang_func = yinianji_nv_feihuoliang_jisuan_fenshu
                Bmi_func = 一年级女生体重指数计算分数(Bmi)
                xuhao = row[0]
                xingming = row[1]
                xingbie = row[2]
                banji = wenjianming
...
def JiSuan_zongchengji(suoyin, quanbanpaiming):
    ...
    if not valid_items_for_rank:
        return quanbanpaiming  # 全部缺考，直接返回
