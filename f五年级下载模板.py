...
def generate_5_grade_template(school_name, header_name, selected_items, boy_count, girl_count, save_path,file_name):
    """
    生成一年级体育成绩模板
    ...
    for i in range(int(boy_count)):
        ...
        if '仰卧起坐' in selected_items:
            ws.cell(row=row, column=7, value=55)
        if '50*8' in selected_items:
            ws.cell(row=row, column=8, value="1'35")
        if '直臂悬垂' in selected_items:
            ws.cell(row=row, column=9, value=65)
        if '肺活量' in selected_items:
            ws.cell(row=row, column=10, value=3000)
