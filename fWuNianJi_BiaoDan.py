...
from fwunianji_jisuanhanshu import JiSuanPingJunFen,JiSuanYouLiangLv,JiSuan_nan_list,JiSuan_nv_list,JiSuan_tianjiapaiming,JiSuan_zongchengji,JiSuan_qudiaobujipaiming,tiaosheng_manfen,JiSuanzhibixuanchuiPingJunFen
from zzhuzhuangtu import wunianji_wuxiang_zhuzhuangtu,wunianji_liuxiang_zhuzhuangtu,wunianji_baxiang_zhuzhuangtu
from zxunliancelue import gao_wu_xunlianfangan_plan,gao_liu_xunlianfangan_plan,gao_ba_xunlianfangan_plan


def wunianji_chuliwenjian(df, origin_name):
    """
    ...
    try:
        ...
        new_file_name = f"五年级体育成绩单_{safe_origin_name}"
        ...
        # ================添加柱状图=================
        wunianji_wuxiang_zhuzhuangtu(sh2)
        # 循环设置 B20~D25 居中 + 边框
        ...
        for row in sh3.iter_rows(min_row=1, max_row=sh3.max_row, min_col=1, max_col=sh3.max_column):
            for cell in row:
                ...
                cell.font = target_font
        # ================添加柱状图=================
        wunianji_liuxiang_zhuzhuangtu(sh3)
        # 循环设置 B20~D25 居中 + 边框
        ...
        for row in sh4.iter_rows(min_row=1, max_row=sh4.max_row, min_col=1, max_col=sh4.max_column):
            for cell in row:
                ...
                cell.font = target_font
        # ================添加柱状图=================
        wunianji_baxiang_zhuzhuangtu(sh4)
        # 循环设置 B20~D25 居中 + 边框
        ...
        #     raise Exception(
        #         "五年级数据处理失败：Excel表格内容错误，请检查【成绩明细】工作表中的单元格数据是否异常！<br>"
        #         f"错误详情：{error_msg}"
        #     )
        if "index" in error_msg:
            raise Exception(
                "五年级数据处理失败：Excel行数/列数不匹配，请检查表格是否少行、少列、格式被修改！"
            )

        else:
            raise Exception(
                f"五年级数据处理失败：Excel表格格式或数据异常，请检查文件是否正确！<br>"
                ...
                "错误信息5. expected string or bytes-like object→→→→→→→→50*8数据有小数点、有空值、需要加 “ ' ”、有非法字符<br>"
