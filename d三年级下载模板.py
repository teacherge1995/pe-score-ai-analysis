...
def generate_3_grade_template(school_name, header_name, selected_items, boy_count, girl_count, save_path,file_name):
    """
    生成三年级体育成绩模板
    ...
    for i in range(int(boy_count)):
        ...
        if '跳绳' in selected_items:
            ws.cell(row=row, column=5, value=180)
        ...
        if '立定跳远' in selected_items:
            ws.cell(row=row, column=8, value=180)
        if '肺活量' in selected_items:
            ws.cell(row=row, column=9, value=2400)
