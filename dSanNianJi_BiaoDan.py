...
from dsannianji_jisuanhanshu import JiSuanPingJunFen,JiSuanYouLiangLv,JiSuan_nan_list,JiSuan_nv_list,JiSuan_tianjiapaiming,JiSuan_zongchengji,JiSuan_qudiaobujipaiming,tiaosheng_manfen
from zzhuzhuangtu import sannianji_sixiang_zhuzhuangtu,sannianji_wuxiang_zhuzhuangtu,sannianji_qixiang_zhuzhuangtu
from zxunliancelue import si_xunlianfangan_plan,zhong_wu_xunlianfangan_plan,qi_xunlianfangan_plan




def sannianji_chuliwenjian(df, origin_name):
    """
    ...
    try:
        ...
        new_file_name = f"三年级体育成绩单_{safe_origin_name}"
        ...
        # ================添加柱状图=================
        sannianji_sixiang_zhuzhuangtu(sh2)
        # 循环设置 B20~D25 居中 + 边框
        ...
        # ================添加柱状图=================
        sannianji_wuxiang_zhuzhuangtu(sh3)
        # 循环设置 B20~D25 居中 + 边框
        ...
        # ================添加柱状图=================
        sannianji_qixiang_zhuzhuangtu(sh4)
        # 循环设置 B20~D25 居中 + 边框
        ...
        #     raise Exception(
        #         "三年级数据处理失败：Excel表格内容错误，请检查【成绩明细】工作表中的单元格数据是否异常！<br>"
        #         f"错误详情：{error_msg}"
        #     )
        if "index" in error_msg:
            raise Exception(
                "三年级数据处理失败：Excel行数/列数不匹配，请检查表格是否少行、少列、格式被修改！"
            )

        else:
            raise Exception(
                f"三年级数据处理失败：Excel表格格式或数据异常，请检查文件是否正确！<br>"
