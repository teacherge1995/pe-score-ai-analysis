...
def generate_6_grade_template(school_name, header_name, selected_items, boy_count, girl_count, save_path,file_name):
    """
    生成一年级体育成绩模板
    ...
    for i in range(int(boy_count)):
        ...
        if '50米' in selected_items:
            ws.cell(row=row, column=4, value=8)
        if '跳绳' in selected_items:
            ws.cell(row=row, column=5, value=210)
        if '坐位体前屈' in selected_items:
            ws.cell(row=row, column=6, value=20)
        if '仰卧起坐' in selected_items:
            ws.cell(row=row, column=7, value=60)
        if '50*8' in selected_items:
            ws.cell(row=row, column=8, value="1'25")
        if '直臂悬垂' in selected_items:
            ws.cell(row=row, column=9, value=75)
        if '肺活量' in selected_items:
            ws.cell(row=row, column=10, value=3500)
        if '身高' in selected_items:
            ws.cell(row=row, column=11, value=150)
        if '体重' in selected_items:
            ws.cell(row=row, column=12, value=34)
    ...
    for col in range(1, 13):  # A-I列
        ...
        cell.border = Border(
            ...
            right=double_side if col == 12 else thin_side  # 最后一列右边框为双线
        )
    ...
    for row in range(5, 33):  # 5-32行
        for col in range(1, 13):  # A-I列
            ...
            right = double_side if col == 12 else thin_side
