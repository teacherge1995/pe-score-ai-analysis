...
def generate_2_grade_template(school_name, header_name, selected_items, boy_count, girl_count, save_path,file_name):
    """
    生成二年级体育成绩模板
    ...
    for i in range(int(boy_count)):
        ...
        if '50米' in selected_items:
            ws.cell(row=row, column=4, value=9)
        if '跳绳' in selected_items:
            ws.cell(row=row, column=5, value=170)
        ...
        if '身高' in selected_items:
            ws.cell(row=row, column=9, value=150)
        if '体重' in selected_items:
            ws.cell(row=row, column=10, value=34)
    ...
    for col in range(1, 11):  # A-I列
        ...
        cell.border = Border(
            ...
            right=double_side if col == 10 else thin_side  # 最后一列右边框为双线
        )
    ...
    for row in range(5, 33):  # 5-32行
        for col in range(1, 11):  # A-I列
            ...
            right = double_side if col == 10 else thin_side
    ...
    ws2['G4'] = '坐位体前屈(cm)'
    ws2['G4'].font = Font(size=11, bold=True)
    ws2['G4'].alignment = Alignment(horizontal='center', vertical='center')
    ws2['G36'] = '坐位体前屈(cm)'
    ws2['G36'].font = Font(size=11, bold=True)
    ws2['G36'].alignment = Alignment(horizontal='center', vertical='center')
    ws2['I4'] = '仰卧起坐(个)'
    ws2['I4'].font = Font(size=11, bold=True)
    ws2['I4'].alignment = Alignment(horizontal='center', vertical='center')
    ws2['I36'] = '仰卧起坐(个)'
    ws2['I36'].font = Font(size=11, bold=True)
    ws2['I36'].alignment = Alignment(horizontal='center', vertical='center')



    # 4. 处理第一个表格区域：A4:I32
    ## 4.1 为第4行（表格标题行）设置双线边框
    for col in range(8, 11):  # A-I列
